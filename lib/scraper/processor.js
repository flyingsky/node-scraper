...
exports.ImageDownloadProcessor = {
  process: function(input, context, callback) {
    ...
    me.downloadImages(images, function(err){
      if (err) {
        return callback(err);
      }
      ...
      // we expand all input image mapping item to array, such as imgSrc maybe an array of image
      // now we get an array of image with downloadPath, we need add imgSrcDownload property with an array
      // of downloadPath
      for(var i in images) {
        var prop = images[i].prop;
        var downloadPath = images[i].success ? images[i].downloadPath : null;
        if (!collectedImages.hasOwnProperty(prop)) {
          collectedImages[prop] = downloadPath;
        } else {
          var existingValue = collectedImages[prop];
          if (util.isArray(existingValue)) {
            existingValue.push(downloadPath);
          } else {
            collectedImages[prop] = [existingValue, downloadPath];
          }
        }
      }

      for (var p in collectedImages) {
        input[p] = collectedImages[p];
      }

      console.log('=========donwload image====', input);
